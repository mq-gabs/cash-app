...
import { TMonthDayView, TMonthView, TYearView } from "../pages/dashboard/types";
...
export function formatMonthView(
  monthView: TMonthView,
  monthIndex: number,
  year: number
): TMonthDayView[] {
...
export function formatYearView(yearView: TYearView): {
  labels: string[];
  series: number[];
} {
  const yearMonthView = Object.values(yearView).reduce(
    (acc, curr, index) => {
      acc.series.push(curr);
      acc.labels.push(months[index]);
